...
Calendar, Tasks, Focus and Habits follow this boundary end to end: pages consume TanStack Query hooks, those hooks receive typed repositories through context, and only the Supabase or explicit development adapter performs remote reads/writes. Profile preferences use the same repository boundary. Task completion updates every matching cached list optimistically, snapshots the previous values, restores them on failure and always reconciles with durable server state.

Calendar stores only authored events. Month, week and day view models project scheduled/deadline Tasks and scheduled Habits at read time, while pure helpers build profile-week ranges, split overnight events into local-day segments and assign deterministic columns to overlap groups. The Habit adapter respects the profile timezone, creation/archive window and daily logs, and produces read-only items that link back to Habits without copying rows into `calendar_events`. Timed coordinates use local wall-clock minutes for display, while durable values remain UTC `timestamptz`. Small screens replace dense time grids with a selected-day agenda rather than shrinking columns beyond usability.
...
The browser router uses feature-level lazy imports. Public auth routes and the protected app shell share an auth provider that resolves either a persisted Supabase session or an explicitly configured local development session before rendering redirects. `public/_redirects` gives Cloudflare Pages an SPA fallback for direct internal-route refreshes.
